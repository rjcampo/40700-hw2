...
oecd_data <- oecd_data %>% 
  select(REF_AREA, MEASURE, SCALE, OBS_VALUE)

col_names <- c(country = "REF_AREA", measure = "MEASURE",
...
```{r, warning = FALSE, message = FALSE}
library(ggplot2)
# Patchwork lets you combine separate plots into a single figure
library(patchwork)
...
for (i in seq_along(trust_vars)) {
  ...
  plot <- ggplot(
    oecd_data, aes(x = .data[[var]], y = TRUST_PE_EXT_CON)) + 
      geom_point(color = "deepskyblue3", alpha = 0.5, shape = 19) + 
      stat_smooth(
        method = "lm",
        se = TRUE,
        level = 0.90,
